...
sync   
    LDR R1, =SSI0_SR_R
	LDR R2, [R1]
	AND R2, #0x10                       ;mask SSI0_SR_R bit 4
	CMP R2, #0x10
	BEQ sync                            ;if bit 4 is high, loop back
	LDR R2, =DC        
	MOV R3, #0x00                      
	STR R3, [R2]                        ;set DC to command mode (0)
	LDR R2, =SSI0_DR_R
	STR R0, [R2]                        ;write given command to the queue
sync2
    LDR R2, [R1]
	AND R2, #0x10
	CMP R2, #0x10 
	BEQ sync2                           ;wait until bit 4 is low to continue
...
syncer
	LDR R1, =SSI0_SR_R
	LDR R2, [R1]
	AND R2, #0x02                   ;mask for bit 1 of SSI0_SR_R
	CMP R2, #0x02
	BNE syncer                      ;if bit 1 is low, loop back
	LDR R2, =DC
	MOV R3, #0x40                   ;set DC to data mode
	STR R3, [R2]
	LDR R2, =SSI0_DR_R
	STR R0, [R2]                    ;write data to the queue
